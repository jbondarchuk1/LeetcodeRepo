...
{
    public partial class ElectricalTestMonitoredDataSheetEditor : DevExpress.XtraEditors.XtraForm, ILabForm, ILabWindow
    {
        public TestForm LabTestForm { get; set; } = null;
        public ElectricalTestMonitoredDataSheet el { get; set; } = null;
        public bool isTablet { get; set; } = false;

        private string _initialContent;
        private string _currentContent;

        public ElectricalTestMonitoredDataSheetEditor()
        {
            InitializeComponent();
        }

        public ElectricalTestMonitoredDataSheetEditor(TestForm f, bool isTab = false) : this()
        {
            this.LabTestForm = f;
            this.isTablet = isTab;
        }
        ...
        private void configure()
        {
            if (this.isTablet)
            {
                ...
                {
                    lblNote1, lblJobNo, lblEngineer, lblCustomer, lblNote2, lblNote2, 
                };
        ...
        public void load()
        {
            ...
            this.el = ElectricalTestMonitoredDataSheet.Load(this.LabTestForm);
			txtJobNo.EditValue= this.el.JobNo;
			txtEngineer.EditValue= this.el.Engineer;
			txtCustomer.EditValue= this.el.Customer;

            _initialContent = ElectricalTestMonitoredDataSheet.Save(this.el);
        ...
        public void Save(bool checkUser = false)
        {
            ...
            this.LabTestForm.Content = ElectricalTestMonitoredDataSheet.Save(this.el);
            
            FormTools.SaveForm<ElectricalTestMonitoredDataSheet, ElectricalTestMonitoredDataSheetEditor>(el, this, ref _initialContent, ref _currentContent, in checkUser);
        ...
        {
            return new ElectricalTestMonitoredDataSheetReport(this.el);
        }

        private void add(GridControl grdControl, GridView grdView)
        {
            // this.el.Data.Add(new ElectricalTestMonitoredDataSheet.TestData());
        ...
        private void ElectricalTestMonitoredDataSheetEditor_Load(object sender, EventArgs e)
        {
            try
            {
                configure();
