...
{
    public partial class SandDataSheetEditor : DevExpress.XtraEditors.XtraForm, ILabForm, ILabWindow
    {
        public TestForm LabTestForm { get; set; } = null;
        public SandDataSheet el { get; set; } = null;
        public bool isTablet { get; set; } = false;
        public SandDataSheetEditor()
        {
            InitializeComponent();
        }

        public SandDataSheetEditor(TestForm f, bool isTab = false) : this()
        {
            this.LabTestForm = f;
            this.isTablet = isTab;
        }
        ...
        public void load()
        {
            this.el = SandDataSheet.Load(this.LabTestForm);
			txtJobNo.EditValue= this.el.JobNo;
			txtDate.EditValue= this.el.Date;
			txtReqTemp.EditValue= this.el.ReqTemp;
			txtReqHum.EditValue= this.el.ReqHum;
			txtReqAirVel.EditValue= this.el.ReqAirVel;
			txtActAirVel.EditValue= this.el.ActAirVel;
			txtReqSandDensity.EditValue= this.el.ReqSandDensity;
			txtActSandDensity.EditValue= this.el.ActSandDensity;
            ...
            if (this.isTablet)
            {
                ...
                {
                    txtJobNo, txtDate, txtReqTemp, txtReqHum, txtReqAirVel, txtActAirVel, txtReqSandDensity, txtActSandDensity, txtEngineer, 
                };
                ...
                {
                    lblJobNo, lblDate, lblReqTemp, lblReqHum, lblReqAirVel, lblftMin, lblActAirVel, lblftMin1, lblReqSandDensity, lblgPerFtCubed, lblActSandDensity, lblftgPerFtCubed1Min, lblEngineer, 
                };
        ...
        public void Save()
        {
			...
			this.el.ReqTemp = txtReqTemp.EditValue.ToString();
			this.el.ReqHum = txtReqHum.EditValue.ToString();
			this.el.ReqAirVel = txtReqAirVel.EditValue.ToString();
			this.el.ActAirVel = txtActAirVel.EditValue.ToString();
			this.el.ReqSandDensity = txtReqSandDensity.EditValue.ToString();
			this.el.ActSandDensity = txtActSandDensity.EditValue.ToString();
			this.el.Engineer = txtEngineer.EditValue.ToString();


            this.LabTestForm.Content = SandDataSheet.Save(this.el);
        ...
        {
            return new SandDataSheetReport(this.el);
        }

        private void add(GridControl grdControl, GridView grdView)
        {
            // this.el.Data.Add(new SandDataSheet.TestData());
        ...
        private void SandDataSheetEditor_Load(object sender, EventArgs e)
        {
            try
            {
                load();
