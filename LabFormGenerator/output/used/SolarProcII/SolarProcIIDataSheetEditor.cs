...
{
    public partial class SolarProcIIDataSheetEditor : DevExpress.XtraEditors.XtraForm, ILabForm, ILabWindow
    {
        public TestForm LabTestForm { get; set; } = null;
        public SolarProcIIDataSheet el { get; set; } = null;
        public bool isTablet { get; set; } = false;
        public SolarProcIIDataSheetEditor()
        {
            InitializeComponent();
        }

        public SolarProcIIDataSheetEditor(TestForm f, bool isTab = false) : this()
        {
            this.LabTestForm = f;
            this.isTablet = isTab;
        }
        ...
        public void load()
        {
            this.el = SolarProcIIDataSheet.Load(this.LabTestForm);
			txtJobNo.EditValue= this.el.JobNo;
			txtTest.EditValue= this.el.Test;
			txtDate.EditValue= this.el.Date;
			txtUnitResTemp.EditValue= this.el.UnitResTemp;
            ...
            if (this.isTablet)
            {
                ...
                {
                    txtJobNo, txtTest, txtDate, txtUnitResTemp, txtCycle, txtEngineer, 
                };
                ...
                {
                    lblJobNo, lblTest, lblDate, lblNote1, lblNote2, lblUnitResTemp, lblCycle, lblof56, lblEngineer, 
                };
        ...
        public void Save()
        {
			...
			this.el.UnitResTemp = txtUnitResTemp.EditValue.ToString();
			this.el.Cycle = txtCycle.EditValue.ToString();
			this.el.Engineer = txtEngineer.EditValue.ToString();


            this.LabTestForm.Content = SolarProcIIDataSheet.Save(this.el);
        ...
        {
            return new SolarProcIIDataSheetReport(this.el);
        }

        private void add(GridControl grdControl, GridView grdView)
        {
            // this.el.Data.Add(new SolarProcIIDataSheet.TestData());
        ...
        private void SolarProcIIDataSheetEditor_Load(object sender, EventArgs e)
        {
            try
            {
                load();
