...
{
    public partial class RainDripRateGenericDataSheetEditor : DevExpress.XtraEditors.XtraForm, ILabForm, ILabWindow
    {
        public TestForm LabTestForm { get; set; } = null;
        public RainDripRateGenericDataSheet el { get; set; } = null;
        public bool isTablet { get; set; } = false;
        public RainDripRateGenericDataSheetEditor()
        {
            InitializeComponent();
        }

        public RainDripRateGenericDataSheetEditor(TestForm f, bool isTab = false) : this()
        {
            this.LabTestForm = f;
            this.isTablet = isTab;
        }
        ...
        public void load()
        {
            this.el = RainDripRateGenericDataSheet.Load(this.LabTestForm);
			txtJobNo.EditValue= this.el.JobNo;
			txtDate.EditValue= this.el.Date;
			txtReqRainfallDrip.EditValue= this.el.ReqRainfallDrip;
			txtRainDripRateTol.EditValue= this.el.RainDripRateTol;
			txtDistFromDripPan.EditValue= this.el.DistFromDripPan;
            ...
            if (this.isTablet)
            {
                ...
                {
                    txtJobNo, txtDate, txtReqRainfallDrip, txtRainDripRateTol, txtDistFromDripPan, txtAvgRainRate5Loc, txtEngineer, 
                };
                ...
                {
                    lblJobNo, lblDate, lblReqRainfallDrip, lblInchPerHour, lblRainDripRateTol, lblDistFromDripPan, lblAvgRainRate5Loc, lblNote1, lblEngineer, 
                };
        ...
        public void Save()
        {
			...
			this.el.ReqRainfallDrip = txtReqRainfallDrip.EditValue.ToString();
			this.el.RainDripRateTol = txtRainDripRateTol.EditValue.ToString();
			this.el.DistFromDripPan = txtDistFromDripPan.EditValue.ToString();
			this.el.AvgRainRate5Loc = txtAvgRainRate5Loc.EditValue.ToString();
			this.el.Engineer = txtEngineer.EditValue.ToString();


            this.LabTestForm.Content = RainDripRateGenericDataSheet.Save(this.el);
        ...
        {
            return new RainDripRateGenericDataSheetReport(this.el);
        }

        private void add(GridControl grdControl, GridView grdView)
        {
            // this.el.Data.Add(new RainDripRateGenericDataSheet.TestData());
        ...
        private void RainDripRateGenericDataSheetEditor_Load(object sender, EventArgs e)
        {
            try
            {
                load();
